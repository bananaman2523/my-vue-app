              ...
              <template v-for="(group, index) in groupedChecklist" :key="index">
                  <tr v-for="(item, i) in group.items" :key="i">
                    ...
                    <td class="border border-gray-300 p-2 text-center">
                        <select v-model="item.status" class="p-2 rounded w-full font-bold text-center" :class="statusClass(item.status)" :disabled="formData.status == 'ผ่าน'">
                    ...
                    <td class="border border-gray-300 p-2 italic text-gray-600 text-center">
                        <textarea v-model="item.note" class="w-full p-1 border rounded text-gray-700" :disabled="formData.status == 'ผ่าน'"></textarea>
                    </td>
                </tr>
              </template>
          </tbody>
